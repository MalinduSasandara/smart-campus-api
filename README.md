...
•	All shared state is a singleton class in a DataStore. Its maps are instances of ConcurrentHashMap, a thread-safe data structure. This guarantees that even with several requests made at the same time JAX-RS can spin up several resource instances at the same time and they can all read and write to a common underlying data without race conditions or data corruption.
